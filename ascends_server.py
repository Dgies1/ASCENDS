...
class ExecuteMLAnalysisHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        try:
            ...
            # Check if we have input_data (from frontend) or path_to_data (from file)
            if "input_data" in json_obj:
                # Handle data sent directly from frontend
                import tempfile
            ...
            response_to_send["MAE"]=float(MAE)
            response_to_send["R2"]=float(R2)
            response_to_send["input_cols"]=input_cols if isinstance(input_cols, list) else [input_cols]
            response_to_send["target_col"]=target_col
            response_to_send["model_abbr"]=model_abbr
            response_to_send["num_fold"]=num_fold
            response_to_send["scaler"]=scaler_option
            ...
            print(f"Error in ExecuteMLAnalysisHandler: {e}")
            import traceback
            traceback.print_exc()
            self.set_status(500)
            self.write(json.dumps({"error": str(e)}))
