...
print("GPUs for Keras: ", end = "")
gpu_list: list = tf.config.list_physical_devices('GPU')
if gpu_list:
    print(gpu_list)
else:
    print("None")
...
def main(args):
    ...
    try:
        ...
        cols_to_remove: list = args.ignore_cols
        target_col: str = args.target_col
        train_cols: list[str] = args.train_cols
        model_type: str = args.model_type
        hyperparameter_file: PurePath = asc.fix_value(args.hyperparameter_file,'PurePath')
        ...
        random_state: int = asc.fix_value(args.random_state,'int')
        feature_selection = args.feature_selection
        feature_selection_file = args.feature_selection_file # TODO: find out of this should be PurePath like hyperparameter_file
        ...
        net_structure: list = args.net_structure
        ...
        net_neuron_max: list = args.net_neuron_max
        ...
        for subfolder in ["predictions","correlations","tests","parameters","graphs","models"]:
            if not os.path.exists(project_folder / subfolder): os.makedirs(project_folder / subfolder)
    ...
    if feature_selection is not None:
        session_number = asc.get_session(project_folder)
        if save_corr_report is not None:
            if save_corr_report == 'True':
                
                save_corr_report = project_folder / "correlations" / ("session" + str(session_number) + train_type + "_" + csv_file.name + "_target=" + target_col + ".csv")
            else:
                save_corr_report = None
            
        if save_corr_chart is not None:
            if save_corr_chart=='True':
                save_corr_chart = project_folder / "correlations" / ("session" + str(session_number) + train_type + "_"+csv_file.name + "_target=" + target_col + ".png")
            else:
        ...
        train_cols = fs_dict[feature_selection]
        ...
        print(" Input columns: %s" %(header_x))
        print(" Target column: %s" %(target_col))     
        print(" Saving correlation report to " + str(project_folder / "correlations" / ("session" + str(session_number) + train_type + "_" + csv_file.name + "_target=" + target_col + ".csv")))
        print(" Saving correlation chart to " + str(project_folder / "correlations" / ("session" + str(session_number) + train_type + "_" + csv_file.name + "_target=" + target_col + ".png")))
    ...
    if auto_tune is True and model_type != 'LR' and model_type != 'LRC':
        ...
        if model_type == 'NET':
            ...
            if hyperparameter_file is not None:
                print (" Warning: %s will be overriden and not be used."%(hyperparameter_file))
            if train_type == 'r':
    ...
    if test is True:
        ...
        try:
            ...
            if model_type == 'NET':
                ...
                if not isinstance(net_structure, list):
                ...
                if train_type == 'r':
                    ...
                    model.fit(x_train, y_train) # TODO: is this necessary
        ...
        project_path = project_folder.parent

        

        if save_metadata is True:
            print(" Saving metadata to " + str(project_folder / "metadata") + ".csv")
            try:
                asc.save_metadata(vars(args), {'MAE':MAE, 'R2':R2,'Accuracy':accuracy}, project_folder)
            except Exception as e:
        ...
        if save_test_chart is True and train_type == 'r':
            print(" Saving test charts to : ", str(project_folder / "graphs" / ("session" +str(session_number) + "r_" + input_name + "_" + model_type + ".png")))
            try:    
                asc.save_comparison_chart(predictions, actual_values, project_folder / "graphs" / ("session" + str(session_number) + "r_" + input_name + "_" + model_type + ".png"))
            except:
        ...
        if save_test_csv is True and train_type == 'r':
            print(" Saving test csv to : ", str(project_folder / "tests" / ("session"+str(session_number) + "r_" + input_name + "_" + model_type + ".csv")))
            try:   
                asc.save_test_data(predictions, actual_values, project_folder / "tests" / ("session"+str(session_number) + "r_" + input_name + "_" + model_type + ".csv"))
            except:
                print(" * Warning: couldn't generate a csv - please make sure the model is properly trained .. ")

        if save_auto_tune is True:
            print(" Saving hyperparameters to file: ", str(project_folder / "parameters" / ("session" + str(session_number) + train_type + "_" + input_name + "_" + model_type + ".tuned.prop")))
            asc.save_parameters(model_parameters, project_folder / "parameters" / ("session"+str(session_number) + train_type + "_" + input_name + "_" + model_type + ".tuned.prop"))
    ...
    Save model
    """
    # TODO: save the model right after training, before evaluating
    # TODO: make sure that no model is getting trained twice by train_and_save
    try:
        ...
        if model_type == 'NET':
            ...
            if train_type=='c':
                ...
                asc.train_and_save_net(model, project_folder / "models" / ("session" + str(session_number) + train_type + "_" + input_name + "_" + model_type + ".pkl"), input_cols = header_x, target_col = header_y, x_train = x_train, y_train = y_train, scaler_option = scaler_option, MAE = MAE, R2 = R2, path_to_save = '.', num_of_folds = num_of_folds, epochs = epochs, batch_siz = batch_size)
        else:
            if train_type == 'r':
                model = asc.define_model_regression(model_type, model_parameters, x_header_size = x_train.shape[1], random_state = random_state)
                asc.train_and_save(model, project_folder / "models" / ("session" + str(session_number) + train_type + "_" + input_name + "_" + model_type + ".pkl"), model_type
                ...
                model = asc.define_model_classifier(model_type, model_parameters, x_header_size = x_train.shape[1], random_state = random_state)
                asc.train_and_save_classifier(model, project_folder / "models" / ("session" + str(session_number) + train_type+"_" + input_name+"_" + model_type + ".pkl"), model_type
                            , input_cols = header_x, target_col = header_y
...
if __name__ == "__main__":
    ...
    parser.add_argument( "project_folder", help = "project folder to write")
    ...
    parser.add_argument( "--ignore_cols", help = "Columns to ignore for training", nargs = '+')
    parser.add_argument( "--random_state", help = "Random seed to shuffle dataset for random values", default = 'None')
    parser.add_argument( "--model_type", choices = ['NET','RF','SVM','NN','RG','LRC','BR','KR','LR', 'XGB'], default = 'RF', help = "LRC (Logistic Regression), RF (Random Forest), SVM (Support Vector Machine), RG (Ridge), NN (k-Nearest Neighbor), or XGB (Extreme Gradient Boosting), RF is selected by default")
